      ...
      <ol className="flex flex-col gap-3">
        {lessons.map((l) => (
          <li key={l.slug}>
            <Link
              href={`/lessons/${l.slug}`}
              className="group flex gap-4 rounded-xl border border-stone-200 bg-white p-5 transition hover:border-stone-400 hover:shadow-sm"
            >
              <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-stone-900 text-sm font-semibold text-white">
                {l.order}
              </div>
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-stone-600">{l.summary}</p>
              </div>
